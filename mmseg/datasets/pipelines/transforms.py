...
#        mask = 1*(np.random.randint(0,3,[hh,ww])>0)
        ...
        if self.offset:
            ...
            img = img * mask + offset
            gt_seg = gt_seg * mask.squeeze() + (1 - mask.squeeze()) * 255 + offset
        else:
            img = img * mask 
            gt_seg = gt_seg * mask.squeeze() + (1 - mask.squeeze()) * 255
        results['img']=img 
        results['gt_semantic_seg'] = gt_seg
        
        # if not os.path.exists("./show_grid/"):
        #     os.makedirs("./show_grid/")
        # p=random.randint(100)
        # cv2.imwrite("./show_grid/"+str(p)+".jpg",results['img'])
        # cv2.imwrite("./show_grid/"+str(p)+".png",results['gt_semantic_seg'])
